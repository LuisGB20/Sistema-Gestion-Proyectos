...
<template>
  <header class="px-6 py-4 flex justify-between items-center bg-white shadow-lg rounded-lg">
    ...
    <div class="flex items-center space-x-6 relative">
      ...
      <div @click.stop="modalAbierto = !modalAbierto"
        class="flex flex-row items-center text-sm font-medium text-white space-x-0
              bg-DarkTeal px-4 py-2 rounded-lg transition duration-300
              hover:bg-LightGray hover:text-white cursor-pointer relative">
        <p>Bienvenido</p>
        <p class="px-2">|</p>
        <p>{{ user?.rol }}</p>
  ...
  class="absolute top-full right-0 mt-4 bg-white shadow-lg rounded-lg w-52 mr-5 z-50 overflow-hidden">
  ...
  <div class="grid grid-rows-1 h-full">
    ...
    <div class="bg-white flex flex-col items-center p-4">
      <button @click="authStore.logout()"
        class="mt-4 w-full text-lg py-2 font-semibold rounded-lg border-2 border-DarkTeal bg-DarkTeal text-white
