...
onBeforeMount(async () => {
  const id = route.params.id as string;  // Obtener el id del proyecto
  console.log("projectId", id);

  try {
    // Asegurarse de pasar el id del proyecto al servicio
    const getProject = await GetProjectById(authStore.employee?.projectId);  
    console.log("getProject", getProject);

    if (getProject.success) {
      project.value = getProject.data.project;
      encharge.value = getProject.data.encharge && getProject.data.encharge.length > 0 
        ? `${getProject.data.encharge[0].employee.name} ${getProject.data.encharge[0].employee.lastName}` 
        : "";
      members.value = getProject.data.employees.map((employee: any) => ({
        name: `${employee.employee.name} ${employee.employee.lastName}`,
        role: employee.roles[0],
        id: employee.id
      }));
      resources.value = getProject.data.project.projectResources.map((resource: any) => ({
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
      ...
      <div class="p-4 rounded-lg bg-white">
        ...
        <div class="grid grid-cols-1 gap-4">
          <div v-for="(resource, index) in resources" :key="index" class="p-4 rounded-lg bg-slate-100">
      ...
      <div class="p-4 rounded-lg bg-white">
        ...
        <div class="grid grid-cols-1 gap-4">
          <div v-for="(member, index) in members" :key="index" class="p-4 flex justify-between rounded-lg bg-slate-100">
            <div>
            ...
            <DeleteUserFromProjectForm :id="member.id" />
          </div>
        </div>
      </div>
    </div>
        ...
        <div v-for="(task, index) in tasks" :key="index" class="p-4 rounded-lg bg-slate-100 mt-4">
          <div @click="toggleTasks(index)" class="cursor-pointer grid grid-cols-1 md:grid-cols-5 gap-4">
            <div class="col-span-3">
              <h3 class="text-md font-semibold cursor-pointer">{{ task.name }}</h3>
              <p class="pl-5">{{ task.description }}</p>
            </div>
            <div class="col-span-2">
              <h3 class="text-md font-semibold cursor-pointer">Integrantes</h3>
          ...
          <div v-if="task.showActivities" class="mt-4" :style="task.activities.length > 4 ? 'max-height: 140px; overflow-y: auto;' : ''">
            ...
            <div class="flex justify-end mt-4">
              <router-link :to="{ name: 'tareas-detalle', params: { id: task.id } }">
                <button class="text-sm text-DarkTeal bg-white px-2 py-2 rounded-2xl" style="max-width: 140px; white-space: normal;">
                  Ver más...
                </button>
              </router-link>
            </div>
